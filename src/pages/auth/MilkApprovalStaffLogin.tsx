...
import { Lock, Scale, Eye, EyeOff } from "lucide-react";
...
const MilkApprovalStaffLogin = () => {
  ...
  const from = (location.state as any)?.from?.pathname || '/milk-approval/dashboard';
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Attempting milk approval staff login for:', loginData.email);
      ...
      if (error) {
        ...
        toast.success('Welcome Back, Approval Staff', 'Accessing milk approval dashboard...');
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-6">
      <div className="w-full max-w-md space-y-8">
        ...
        <div className="text-center space-y-4">
          ...
          <div>
            <h1 className="text-3xl font-heading font-bold">Milk Approval Portal</h1>
            <p className="text-muted-foreground mt-2">Access milk approval dashboard and tools</p>
          </div>
        </div>
        ...
        <Card className="border-2 border-primary/10">
          <CardHeader className="space-y-1">
            <CardTitle className="text-2xl text-center">Milk Approval Staff Login</CardTitle>
            <CardDescription className="text-center">
              Secure access for milk approval staff members
            </CardDescription>
          </CardHeader>
          <CardContent>
...
export default MilkApprovalStaffLogin;