...
import { Loader2, Milk, Eye, EyeOff, User } from 'lucide-react';
...
export default function StaffLogin() {
  ...
  const dummyCredentials = {
    worker: { username: 'worker', password: 'worker123', role: 'Field Worker' },
    worker2: { username: 'worker2', password: 'worker123', role: 'Field Worker' },
  ...
  const handleDummyLogin = (username: string, password: string) => {
    ...
    setTimeout(() => {
      const form = document.getElementById('staff-login-form') as HTMLFormElement;
      if (form) {
        form.requestSubmit();
      }
    }, 100);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-50 p-4">
      <Card className="w-full max-w-md shadow-xl border-0 bg-white/80 backdrop-blur-sm">
        ...
        <CardContent>
          <form id="staff-login-form" onSubmit={handleSubmit} className="space-y-4">
            {error && (
