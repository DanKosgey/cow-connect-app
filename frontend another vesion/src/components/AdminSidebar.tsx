              ...
              {menuItems.map((item) => (
                <SidebarMenuItem key={item.title}>
                  ...
                  >
                    <Link 
                      to={item.url} 
                      className="flex items-center space-x-3 text-gray-700 hover:text-green-700 transition-colors" 
                      onClick={() => handleNavigation(item.url)}
                      aria-current={isMenuItemActive(item.url) ? "page" : undefined}
                    >
        ...
        <div className="flex items-center space-x-3 mb-4">
          <Avatar className="h-8 w-8">
            <AvatarFallback className="bg-gradient-to-br from-green-600 to-emerald-600 text-white text-sm">
              {user?.user_metadata?.full_name ? user.user_metadata.full_name.charAt(0) : 'AD'}
            </AvatarFallback>
          </Avatar>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-gray-900">{user?.user_metadata?.full_name || 'Farm Administrator'}</p>
            <p className="text-xs text-green-600 truncate">{user?.email || 'admin@dairychain.com'}</p>
          </div>
        </div>
