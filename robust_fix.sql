-- Robust fix for missing tables and columns
...
BEGIN
  IF NOT EXISTS (
    ...
    ADD COLUMN collection_point_id uuid REFERENCES public.collection_points(id) ON DELETE SET NULL;
  END IF;
END$$;
...
-- 7. Handle RLS policies for collection_points (more robust approach)
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tbl.relname = 'collection_points' AND pol.polname = 'Collection points are viewable by authenticated users'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy pol
    JOIN pg_class tbl ON pol.polrelid = tbl.oid
    WHERE tbl.relname = 'collection_points' AND pol.polname = 'Collection points are insertable by admins'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy pol
    JOIN pg_class tbl ON pol.polrelid = tbl.oid
    WHERE tbl.relname = 'collection_points' AND pol.polname = 'Collection points are updatable by admins'
  ) THEN
...
-- 8. Handle RLS policies for variance_penalty_config (more robust approach)
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy pol
    JOIN pg_class tbl ON pol.polrelid = tbl.oid
    WHERE tbl.relname = 'variance_penalty_config' AND pol.polname = 'Variance penalty configs are viewable by authenticated users'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy pol
    JOIN pg_class tbl ON pol.polrelid = tbl.oid
    WHERE tbl.relname = 'variance_penalty_config' AND pol.polname = 'Variance penalty configs are manageable by admins'
  ) THEN
