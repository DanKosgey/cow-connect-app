...
BEGIN
    ...
    SELECT 
        ...
        ce.period_start,
        ce.period_end,
        ce.total_collections,
        ce.total_liters,
        ce.rate_per_liter,
        ce.total_earnings,
        COALESCE(cp.total_penalty, 0)::NUMERIC(10,2) as total_penalties,
        GREATEST(0, ce.total_earnings - COALESCE(cp.total_penalty, 0))::NUMERIC(10,2) as adjusted_earnings
    FROM collector_earnings ce
    LEFT JOIN collector_penalties cp ON ce.collector_id = cp.staff_id;
END;
$$ LANGUAGE plpgsql;
