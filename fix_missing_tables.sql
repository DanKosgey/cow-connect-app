-- Fix for missing tables and columns
-- Run this in your Supabase SQL Editor

-- 1. Add the missing collection_point_id column to collections table
ALTER TABLE public.collections 
ADD COLUMN IF NOT EXISTS collection_point_id uuid REFERENCES public.collection_points(id) ON DELETE SET NULL;

-- 2. Create the collection_points table if it doesn't exist
...
-- 3. Create the variance_penalty_config table if it doesn't exist
...
-- 4. Create the variance_type_enum if it doesn't exist
DO $$ 
BEGIN
...
-- 7. Add RLS policies for collection_points
...
-- 8. Add RLS policies for variance_penalty_config
